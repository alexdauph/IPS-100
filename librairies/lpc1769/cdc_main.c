...
static void usb_pin_clk_init(void)
{
	...
	Board_USBD_Init(1);															//* SIMPLE BUT IMPORTANT TO SET D+ and D- as GPIO

	Chip_IOCON_PinMux(LPC_IOCON, 2, 9, IOCON_MODE_INACT, IOCON_FUNC0);			//* IMPORTANT
	Chip_GPIO_SetPinDIROutput(LPC_GPIO, 2, 9);									//* IMPORTANT
	Chip_GPIO_SetPinOutLow(LPC_GPIO, 2, 9);										//* IMPORTANT
}
...
int main(void)
{
	...
	USB_CORE_DESCS_T desc;	ErrorCode_t ret = LPC_OK;
	uint32_t prompt = 0, rdCnt = 0;

	/* Initialize board and chip */
	SystemCoreClockUpdate();
	Board_Init();									//SystemCoreClock = 25MHz ???

	StopWatch_Init();								//Probably not useful to vCOM
